...
class SendEmailSpec extends AnyFlatSpec with Matchers {
  ...
  val rows = Seq(
    ("127.0.0.1", "27/Dec/2037:12:00:00 +0530", "Hello chat", "GET", "HTTP/1.1", 200, 1024, "", "Mozilla/5.0", 100)
  )
  ...
  "SendEmail" should "send email successfully" in {
    val isUserPromptAnomaly = false // Set to true if it's a user prompt anomaly
    SendEmail.send(df.head, isUserPromptAnomaly)
