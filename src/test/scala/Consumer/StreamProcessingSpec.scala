...
class StreamProcessingSpec extends AnyFlatSpec with Matchers with BeforeAndAfterAll {
  ...
  "StreamProcessing" should "send anomalies to ElasticSearch" in {
    // Input Kafka message
    val jsonString = """{"ipAddress":"127.0.0.1","dateTime":"27/Dec/2037:12:00:00 +0530","request":"GET","prompt":"test prompt","protocol":"HTTP/1.1","status":200,"bytes":200000,"referrer":"-","userAgent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.146 Safari/537.36","responseTime":80000}"""
    val rdd = spark.sparkContext.parallelize(Seq(jsonString))
    val df = spark.read.json(rdd)

    // Call StreamProcessing.classifyAnomalies()
